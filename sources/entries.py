...
class Entry:
    ...
    def try_to_add_image(self):
        print('try to add image')
        if self.image:
            return
        if redis.exists(self.url):
            self.image = str(redis.get(self.url), 'utf-8')
            print('found in cache : ', self.image)
            return
        try:
            self.image = get_image_of_url(self.url)
            print(self.image, 'found for', self.url)
            redis.set(self.url, self.image, ex=864000) # 10 days of cache
        except:
            print("erreur")
            #redis.set(self.url, '', ex=864000) # 10 days of cache
...
def retrieve_rss(rss):
    """
    Request of the rss content from cache (redis) or from the web
    """
...
def get_image_of_url(url):
    r = requests.get(url=url)
    soup = BeautifulSoup(r.text, 'html.parser')
    res = soup.find('meta', attrs={'property': 'og:image'})
